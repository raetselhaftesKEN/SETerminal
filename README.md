...
2. 物理引擎的使用。各个物体之间的碰撞消息均使用物理引擎发出，而在物理引擎表现不佳的障碍物类上，又通过自主设计碰撞算法解决了问题。
3. 丰富的枪支道具种类。
4. 一个能使敌人不停移动与射击的亮眼回调递归。
5. 利用Const.h管理全局常量。
6. 设计了一个独立于cocos体系之外的单例客户端类。
7. 全面且简洁的UI界面与动画，保证视觉效果的同时展示全部玩家所需要知道的信息
8. 能够移植到Android平台。
